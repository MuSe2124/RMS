<%-- 
    ...
    Created on : 27 Jun 2022, 09:19:45
    Author     : HP
--%>
...
<html>
    ...
    <div id="reportpage" class="mid">
        ...
        <%if(report!=null){%>
        <h2>Table of Monthly Sales</h2><br>
        <table style="width:100%">
            ...
            <tr>
                 <%float amount=0;for(LineItem ll:ss.getLineItems()){
                    amount+=ll.getTotal();}%>
                    <%colors.add("rgb("+(int)Math.random()*256+","+(int)Math.random()*256+","+(int)Math.random()*256+")");%>
                <td> <%= ss.getStore().getId() %> </td>
                <td> <%= ss.getEmployee().getId() %> </td>
                <td><%=ss.getDate()%></td>
                <td>amount</td>
                <%x.add(ss.getStore().getId());y.add(amount);%>
            </tr>
            <%}%>
        </table>
        <br>
        ...
        <a onclick="this.href='data:text/html;charset=UTF-8,'+encodeURIComponent(document.documentElement.outerHTML)" href="monthlySales.pdf" download="monthlySales.pdf">Download Report</a></p>
        <%}%>
    </div>    
    </form
